package com.example.demo.A_msb.io.testreactor;
...
//        try {
//            s.register(st.selector, SelectionKey.OP_ACCEPT);  //如果selector阻塞在selector.select()，此步会被阻塞的!!!!!
//            st.selector.wakeup();  //功能是让 selector的select（）方法，立刻返回，不阻塞！
        //  wakeup放在register的前后都不合理，都会出现并发问题，在前，可能刚唤醒selector.select()，还没来及register又执行到selector.select()
        // 在后，可能这里的register就直接阻塞了。所以应该用队列解耦，交给线程自己去register。见nextSelectorV3
